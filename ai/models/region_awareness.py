...
class TemporalSelfAttention(nn.Module):
    def __init__(self, dim: int = 2816, nhead: int = 4, dropout: float = 0.1):
        super().__init__()
    ...
    def forward(self, x):
        # x: [B, C, T] → transpose để attention theo thời gian
        x = x.transpose(1, 2)         # [B, T, C]
        x = self.attn(x)              # [B, T, C]
        x = x.transpose(1, 2)         # [B, C, T] (giữ nguyên shape đầu vào)
        return x
...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x, feature):
        ...
        for i in range(len(x[0])):
            ...
            #print("features_stack_shape: ", features_stack.shape)
            features_stack = self.temporal_attention(features_stack)  # [B, C, T] -> [B, T, C] -> [B, C, T]
            
            weights_stack = torch.stack(weights, dim=2)
            weights_stack = softmax(weights_stack, dim=2)
            #print("weights_stack_shape: ", weights_stack.shape)
