...
from mobilenetv3 import mobilenetv3_small  # dùng model large → output 960


class AV_MobileNetV3(nn.Module):
    def __init__(self, pretrained_mobilenetv3=True):
        super(AV_MobileNetV3, self).__init__()
        self.mobilenet = mobilenetv3_small()
        if pretrained_mobilenetv3:
            self.mobilenet.load_state_dict(torch.load('pretrained/mobilenetv3-small-55df8e1f.pth'))
    ...
    def forward(self, x, feature):
        ...
        for t in range(len(x[0])):  # Loop over time steps
            ...
            for b in range(len(x)):  # Loop over batch
                f = self.mobilenet(x[b][t])       # [1, 960, 1, 1]
